...
export function formatDashboardMarkdown(dashboard: ClarityDashboard): string {
  return [
    ...
    `- Average Clarity Score: ${dashboard.averageScore}/100`,
    `- Good tasks: ${dashboard.quality.good}`,
    `- Medium tasks: ${dashboard.quality.medium}`,
    `- Poor tasks: ${dashboard.quality.poor}`,
    '',
    '## Top Problems',
    '',
    ...(topItems(dashboard.topProblems).length > 0 ? topItems(dashboard.topProblems) : ['No recurring problems found.']),
    '',
    '## Lowest Score Tasks',
    '',
...
export function formatResearchReportMarkdown(data: ResearchReportData): string {
  return [
    '# Clarity Guardian Research Notes',
    '',
    '## Hypothesis',
    '',
...
export function formatDashboardHtml(dashboard: ClarityDashboard): string {
  ...
  return [
    ...
    `<p>Generated at ${dashboard.generatedAt}</p>`,
    '<section class="metrics">',
    `<div class="metric"><div>Total tasks</div><div class="value">${dashboard.totalTasks}</div></div>`,
    `<div class="metric"><div>Average score</div><div class="value">${dashboard.averageScore}</div></div>`,
    `<div class="metric"><div>Good</div><div class="value">${dashboard.quality.good}</div></div>`,
    `<div class="metric"><div>Medium</div><div class="value">${dashboard.quality.medium}</div></div>`,
    `<div class="metric"><div>Poor</div><div class="value">${dashboard.quality.poor}</div></div>`,
    '</section>',
    '<h2>Top problems</h2>',
    `<table><thead><tr><th>Problem</th><th>Count</th></tr></thead><tbody>${problemRows}</tbody></table>`,
    '<h2>Lowest score tasks</h2>',
    `<table><thead><tr><th>Task</th><th>Title</th><th>Score</th><th>Risk</th></tr></thead><tbody>${lowScoreRows}</tbody></table>`,
    '</main></body></html>'
  ].join('');
}
