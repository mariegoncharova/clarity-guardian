...
function detectBrokenWithoutReproductionSteps(body: string): Remark | null {
...
export function analyzeRequiredSections(body: string): Remark[] {
...
export function analyzeStopPhrases(body: string): Remark[] {
  const remarks: Remark[] = [];
  const normalized = normalizeText(body).toLowerCase();
...
function buildManagerChecklistComment(task: NormalizedTask, remarks: Remark[]): string {
...
export function analyzeTask(task: TaskPayload): AnalysisResult {
